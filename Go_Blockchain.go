...
func (b *Block) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		PreviousHash [32]byte       `json:"previouts_hash"`
...
func (bc *Blockchain) ValidProof(nonce int, previousHash [32]byte, transactions []*Transaction, defficulty int) bool {
	zeros := strings.Repeat("0", defficulty)
	guessBlock := Block{0, nonce, previousHash, transactions}
	guessHashStr := fmt.Sprintf("%x", guessBlock.Hash())
	return guessHashStr[:defficulty] == zeros
}
...
func (t *Transaction) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		Recipient string  `"json:"recipinet_blockchain_address"`
...
func main() {
	...
	blockChain.CreatBlock(nonce, previousHash)
	blockChain.Print()
